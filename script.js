/* ===== Wedding Invite — Shared Logic (Flat structure, no folders) ===== */
/* يعتمد على وجود window.WEDDING_CONFIG محمّل قبل هذا الملف */
...
(function () {
  ...
  const API_BASE = '/rsvp'; // Cloudflare Pages Function (functions/rsvp.js)
    ...
    try {
      ...
      if (guestListEl && Array.isArray(data.recent)) {
        guestListEl.innerHTML = data.recent
          .map(
            (g) =>
              `<div class="guest-list-item">${escapeHtml(g.name)} — ${g.guestsCount} ${
                g.guestsCount > 1 ? 'أشخاص' : 'شخص'
              }</div>`
          )
          .join('');
      }
